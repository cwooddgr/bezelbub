...
> | Glama | no, **skipped** (decided-by-user 2026-09-11) | Charlie opened the Add Server form (name, description, public GitHub repo URL, then GitHub OAuth) and chose not to submit. Their post-approval step asks for a Dockerfile for automated checks that run on Linux, and only servers passing those checks get indexed for search; ours needs the macOS-native bezelbub binary, so it would be listed but not searchable. Not worth the effort. |
> | Smithery | **yes, since 2026-09-11** | `charlie-wood/bezelbub-mcp` (https://smithery.ai/servers/charlie-wood/bezelbub-mcp), published from the CLI after Charlie approved the browser login. Smithery's publish API rejects tool entries without `inputSchema` (three "expected object, received undefined" errors, one per tool), while the MCPB manifest schema rejects `inputSchema` on tool entries; `npm run bundle:smithery` (`scripts/build-smithery-bundle.mjs`) packs a clean bundle and swaps in a manifest with the live schemas. Listing description came through empty (the server-card payload has no description field); fill it in on the Smithery site if it matters. Namespace `charlie-wood` was the one Smithery created on login; a DGR Labs namespace was not created. |
>
